...
class Bungiesearch(Search):
    ...
    _model_to_index = defaultdict(list)
    _model_name_to_index = defaultdict(list)
    _model_name_to_model_idx = defaultdict(list)
    _index_to_model = defaultdict(list)
    _idx_name_to_mdl_to_mdlidx = defaultdict(dict)
    ...
    def __load_settings__(cls):
        ...
        # Loading indices.
        for index_prefix, module_str in cls.BUNGIE['INDICES']:
            ...
            for index_obj in itervalues(index_module.__dict__):
                try:
                    if issubclass(index_obj, ModelIndex) and index_obj != ModelIndex:
                        ...
                        if index_prefix:
                            index_name = "_".join((index_prefix, index_name))
                        ...
                        if index_instance.is_default:
                            if assoc_model.__name__ in cls._model_name_to_default_index:
                                raise AttributeError('ModelIndex {} on index {} is marked as default, '
                                                     'but {} was already set as default.'
                                                     .format(index_instance, index_name,
                                                             cls._model_name_to_default_index[assoc_model.__name__]))
    ...
    @classmethod
    def _build_key(cls, urls, timeout, **kwargs):
        ...
        kwargs = sorted(kwargs.items(), key=lambda item: item[0])
        kwargs = repr([(k, v) for k, v in kwargs])
    ...
    def get_index(cls, model, via_class=False):
        ...
        try:
            ...
            raise KeyError('Could not find any index defined for model {}. Is the model in one of the model index '
                           'modules of BUNGIESEARCH["INDICES"]?'.format(model))
    ...
    def get_models(cls, index, as_class=False):
        ...
        try:
            ...
            raise KeyError('Could not find any index named {}. '
                           'Is this index defined in BUNGIESEARCH["INDICES"]?'.format(index))
    ...
    def map_raw_results(cls, raw_results, instance=None):
        ...
        :param instance: Bungiesearch instance if you want to make use of `.only()`
            or `optmize_queries` as defined in the ModelIndex.
        ...
        results = [None] * len(raw_results.hits if hasattr(raw_results, 'hits') else raw_results)

        found_results = {}
        for pos, result in enumerate(raw_results):
            model_name = result.meta.doc_type
            if (model_name not in Bungiesearch._model_name_to_index
                    or result.meta.index not in Bungiesearch._model_name_to_index[model_name]):
                logger.warning('Returned object of type {} ({}) is not defined in the settings, '
                               'or is not associated to the same index as in the settings.'.format(model_name, result))
                results[pos] = result
            else:
        ...
        for ref_name, ids in iteritems(model_results):
            ...
            if instance:
                ...
                # Prevents setting the database fetch to __fields
                # but not having specified any field to elasticsearch.
                if desired_fields:
                    items = items.only(
                        *[field.name for field in model_obj._meta.get_fields()
                          ...
                          if (field.name in desired_fields
                              and not (field.many_to_one and field.related_model is None))]
                    )
    ...
    def __init__(self, urls=None, timeout=None, force_new=False, raw_results=False, **kwargs):
        """ Creates a new ElasticSearch DSL object.

        Grabs the ElasticSearch connection from the pool
        ...
        :param urls: A list of URLs, or a single string of URL (without leading `http://`),
            or None to read from settings.
        :param idx: A list of indices or a single string representing an index_name name.
            Is optional. Will be merged with `idx_alias`.
        :param idx_alias: A list of index_name aliases or a single string representing an index_name alias,
            as defined in the settings. Will be merged with `index_name`.
        :param timeout: Timeout used in the connection.
        :param force_new: Set to `True` to force a new elasticsearch connection.
            Otherwise will aggressively use any connection with the exact same settings.
        :param **kwargs: Additional settings to pass to the low level elasticsearch client
            and to elasticsearch-sal-py.search.Search.
        """
    ...
    def only(self, *fields):
        """
        Restricts the fields to be fetched when mapping.
        Set to `__model` to fetch all fields define in the ModelIndex.
        """
        s = self._clone()
    ...
    def __getitem__(self, key):
        """ Overwriting the step in slice.

            It is used to set the results either as elasticsearch-dsl-py response object, or
            attempt to fetch the Django model instance.
            :warning: Getting an item will execute this search.
                Any search operation or field setting *must* be done prior to getting an item.
        """
    ...
    def hook_alias(self, alias, model_obj=None):
        ...
        try:
            ...
            raise AttributeError('Could not find search alias named {}. '
                                 'Is this alias defined in BUNGIESEARCH["ALIASES"]?'.format(alias))
        else:
            if (search_alias._applicable_models
                and ((model_obj and model_obj not in search_alias._applicable_models)
                     or not any([app_model_obj.__name__ in self._doc_type
                                 for app_model_obj in search_alias._applicable_models]))):
                    raise ValueError('Search alias {} is not applicable to model/doc_types {}.'
                                     .format(alias, model_obj if model_obj else self._doc_type))
